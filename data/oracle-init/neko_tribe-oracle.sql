...
BEGIN
    -- 重置所有标签趋势状态
    UPDATE n_hashtags SET is_trending = 0, trending_score = 0;
...
-- :page - 页码（从1开始）
-- :pagesize - 每页记录数
-- 例如：page=1, pagesize=20 表示获取第1页，每页20条记录
--      page=2, pagesize=20 表示获取第2页，每页20条记录
*/

-- Oracle中视图不支持COMMENT
-- COMMENT ON VIEW v_user_timeline IS '用户时间线视图，包含自己和关注用户的推文';
...
-- COMMENT ON VIEW v_notifications_summary IS '用户通知摘要视图，用于快速获取用户通知统计信息';
...
FROM (
    ...
    FROM v_home_timeline h
    WHERE (h.viewer_id = :user_id OR h.author_id = :user_id)
      AND fn_can_view_tweet(:user_id, h.tweet_id) = 1
)
WHERE rn > (:page - 1) * :pagesize 
  AND rn <= :page * :pagesize
ORDER BY created_at DESC;
...
FROM (
    SELECT 
        v.*,
        ROW_NUMBER() OVER (ORDER BY v.engagement_score DESC, v.created_at DESC) AS rn
    FROM v_trending_tweets v
...
-- 根据类型参数获取不同的推文列表
-- 使用示例：
-- EXEC sp_get_tweet_list('home', :user_id, :page, :pagesize);
-- EXEC sp_get_tweet_list('user', :user_id, :page, :pagesize, :target_user_id);
-- EXEC sp_get_tweet_list('my_tweets', :user_id, :page, :pagesize);
-- EXEC sp_get_tweet_list('trending', :user_id, :page, :pagesize);

-- 分页参数说明：
-- :page - 页码（从1开始），例如：1, 2, 3...
-- :pagesize - 每页记录数，例如：10, 20, 50
-- 计算偏移量：OFFSET = (:page - 1) * :pagesize
-- 示例：page=3, pagesize=20 表示跳过前40条记录，取接下来的20条
*/
