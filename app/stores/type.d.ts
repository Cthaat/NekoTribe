...
export interface UserPreference {
  ...
  // 6. 其他可扩展变量
  lastSeenTweetId?: number; // 上次访问首页时看到的推文ID（可选）
  lastSeenNotificationId?: number; // 上次查看通知的ID（可选）
  mute_keywords?: string[]; // 屏蔽关键词列表（可选）
  blocked_users?: number[]; // 屏蔽用户ID列表（可选）

  // 7. 用户登录相关
  user: CurrentUserVM; // 当前用户
  authSession: AuthSessionVM | null; // 当前会话信息

  // 更新时间戳等辅助字段
  updatedAt?: string; // ISO 时间字符串（可选）
}
