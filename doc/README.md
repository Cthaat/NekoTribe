...
本仓库已按“新开发者克隆后可快速启动”的目标整理：复制并检查环境变量样例、启动 Docker 依赖并由 Compose 自动初始化 Oracle、运行本地开发服务即可进入联调。
...
2. `server/utils/api-cache.ts` 用带路由、用户和标签版本的 Redis key 缓存 v2 GET 响应。
3. `server/utils/v2.ts` 在 v2 GET 请求中先查 Redis，未命中时再访问 Oracle，并把成功响应写入带 TTL 的缓存。
4. v2 写接口成功后会提升相关缓存标签版本，让旧数据不再被命中并自然过期。
5. `server/utils/redis.ts` 为 WebSocket 提供发布者、订阅者和降级能力。
6. `server/routes/_ws.ts` 维护连接会话、房间、广播和心跳。
7. 多实例部署时，Redis pub/sub 用于跨实例广播。
...
| 缓存与实时能力 | Redis 7，通过 `ioredis` 提供 v2 API 响应缓存、验证码/限流状态和 WebSocket pub/sub；Nitro WebSocket 路由为 `server/routes/_ws.ts` |
...
- 如果把 `ORACLE_IMAGE` 切换为官方 Oracle 镜像，需要 Oracle Container Registry 访问权限并接受镜像协议。
- 可选：如果在 Docker 外运行旧版 v1 媒体缩略图功能，需要本机安装 `ffmpeg`。

## 快速开始前: 配置 `.env`

先复制环境变量模板，再按启动方式核对必填项。`.env` 里包含密钥、数据库、Redis、SMTP、端口和 SentimentFlow 配置；如果只是执行命令但没有检查这些字段，常见结果是认证失败、Oracle 连接失败、验证码邮件发不出去，或 Docker 容器内误连宿主机地址。
...
- SentimentFlow 后端 API：`http://localhost:8846/health`、`http://localhost:8846/docs`
- SentimentFlow 前端界面：`http://localhost:30008`
- Oracle 监听端口：`localhost:5501`
- Oracle EM Express：`http://localhost:5500/em`

Oracle 首次启动通常需要数分钟。`.env.example` 默认使用国内 Oracle 19c 镜像；如果你把 `ORACLE_IMAGE` 切换为官方镜像，请先执行 `docker login container-registry.oracle.com`，并在 Oracle Container Registry 接受数据库镜像协议。也可以改用外部 Oracle 实例，并同步更新 `.env` 中的 `ORACLE_HOST`、`ORACLE_PORT`、`ORACLE_SERVICE_NAME`、`ORACLE_USER` 和 `ORACLE_PASSWORD`。

## SentimentFlow 运行方式

两个 Compose 文件都会包含 SentimentFlow，并拆成两个服务：

- `sentimentflow`：FastAPI 后端，提供预测、训练、模型管理和统计接口。
- `sentimentflow-frontend`：Next.js 前端界面，通过 `BACKEND_API_URL` 访问后端。
...
### Docker 部署：拉取 GHCR 镜像

默认部署使用 `docker-compose.yml`，直接拉取 GitHub Container Registry 上的自动构建镜像：

- 后端：`ghcr.io/cthaat/sentimentflow-backend:latest`
- 前端：`ghcr.io/cthaat/sentimentflow-frontend:latest`
...
- SentimentFlow 后端健康检查：`http://localhost:8846/health`
- SentimentFlow 后端接口文档：`http://localhost:8846/docs`
- SentimentFlow 前端界面：`http://localhost:30008`
...
适用于 NekoTribe 在宿主机运行、Oracle/Redis 用 Docker，或者你想手动启动 SentimentFlow 的场景。

1. 启动 NekoTribe 宿主机开发依赖：
...
4. NekoTribe 的 `.env` 中，使用默认后端宿主机端口时保持 `SENTIMENTFLOW_BASE_URL` 为空；如果后端在其他地址，则显式写入完整 `/api` 地址：
...
然后运行 `yarn dev`。检查 `http://localhost:8846/health`、`http://localhost:8846/docs`、`http://localhost:30008`，以及 NekoTribe 内的 SentimentFlow 集成功能。
...
| Redis 7                   | v2 API 响应缓存、v1 验证码存储、限流状态、WebSocket 发布/订阅和实时广播。 | `docker compose up -d redis`。                                                                                          | `.env` 中的 `REDIS_*`。                                                                                                                                                                                   |
...
在 `.env` 里通过 `ORACLE_IMAGE` 选择 Oracle 镜像来源。`.env.example` 默认使用 `registry.cn-hangzhou.aliyuncs.com/zhuyijun/oracle:19c`。如果切换为 `container-registry.oracle.com/database/enterprise:19.3.0.0`，需要先登录 Oracle Container Registry 并接受镜像协议。
