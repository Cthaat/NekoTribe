...
备注：`doc/neko_tribe-oracle-v2.sql` 是当前 v2 开发数据库基线。`data/oracle-init/` 下的文件保留为历史群组模块脚本，主 compose 不再自动挂载它们，因为这些脚本本身不是完整数据库基线。
...
### `docker compose up -d --build` 提示 `dependency failed to start`，但稍后 `app` 变成 `healthy`
