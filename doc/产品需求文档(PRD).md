...
- [NekoTribe长期最终可执行路线图](./NekoTribe长期最终可执行路线图.md)

该路线图基于当前仓库真实状态重新排序，重点不再是简单堆叠新功能，而是先完成 `API v2`、数据收口、原型模块转正式、质量闸门、运营闭环，再逐步推进移动端、国际化、AI 和商业化。

### 14.1 阶段总览（2026-05-01 至 2028-04-30）

| 阶段 | 时间 | 目标 |
|------|------|------|
| Phase 0 | 2026-05 | 基线冻结、接口盘点、页面状态盘点、数据库差异盘点 |
| Phase 1 | 2026-06 至 2026-07 | 建立 `server/api/v2/**`、打通 v1/v2 并行与首批只读迁移 |
| Phase 2 | 2026-08 至 2026-10 | 完成 auth/session、settings/sessions/statements、posts/tags/media 的正式收口 |
| Phase 3 | 2026-11 至 2027-01 | 将群组、审核、聊天、搜索等模块从原型态升级为内测可用版本 |
| Phase 4 | 2027-02 至 2027-06 | 发布公测版，建立增长、运营、监控、风控闭环 |
| Phase 5 | 2027-07 至 2027-10 | 发布 Web `V1.0`，推进 PWA/移动适配，完成 v1 下线收尾 |
| Phase 6 | 2027-11 至 2028-04 | 推进国际化、AI 辅助能力和轻量商业化验证 |

### 14.2 执行原则

1. 2026 年优先解决架构与数据问题，不无序增加高成本新功能。
2. 2027 年上半年完成“可真实运营”的产品闭环，再扩大用户规模。
3. 移动端、国际化、AI、商业化必须建立在 Web 主线稳定之后推进。

### 14.3 近期 90 天优先动作

1. 盘点所有 `v1` 接口、页面状态和数据库对象差异。
2. 创建 `server/api/v2/` 基础目录并建立统一响应工具。
3. 以 `users/me` 跑通第一条 v1/v2 平滑迁移链路。
4. 启动 `settings/sessions/statements` 从本地存储迁移到 Oracle 的设计与实施。

## 15. 附录

### 15.1 相关文档
- [长期最终可执行路线图](./NekoTribe长期最终可执行路线图.md)
