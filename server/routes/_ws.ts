...
function initializeRedisSubscriptions() {
  ...
  // 订阅系统通知频道
  // 用于接收系统级别的通知消息
  subscribeToChannel(REDIS_CHANNELS.SYSTEM_NOTIFICATION, (message: WSMessage) => {
...
async function handleJoinRoom(sessionId: string, data: any) {
  ...
  if (success && session) {
    ...
    await publishMessage(`${REDIS_CHANNELS.ROOM_MESSAGE}${roomId}`, joinMessage)
...
async function handleLeaveRoom(sessionId: string, data: any) {
  ...
  if (success && session) {
    ...
    await publishMessage(`${REDIS_CHANNELS.ROOM_MESSAGE}${roomId}`, leaveMessage)
...
async function handleRoomMessage(sessionId: string, data: any) {
  ...
  if (!session || !session.joinedRooms.has(roomId)) {
    // 如果用户不在房间中，发送错误消息
    session?.peer.send(JSON.stringify({
      type: 'error',
