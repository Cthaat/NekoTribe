export default defineEventHandler(async event => {
  ...
  // 获取 query 参数
  const query: TweetRepliesPayload = getQuery(
    event
  ) as TweetRepliesPayload;
  ...
  try {
    ...
    // 获取总数
    const ReplyCountResult = await connection.execute(
      getReplyTweetCountSql,
      {
        tweetId,
        user_id: user.userId
      }
    ...
    // 提取总数
    const totalCount: number = ReplyCountResult
      .rows[0][0] as number;
