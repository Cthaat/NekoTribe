...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    const checkResult = await connection.execute(checkSql, { userId: decoded.userId, refreshToken: refreshToken });
    const checkSessionResultRow: checkSessionResultRow = checkResult.rows?.[0] as checkSessionResultRow || [];
    ...
    WHERE user_id = :userId AND DBMS_LOB.COMPARE(refresh_token, :refreshToken) = 0 AND is_active = 1
    `;
    const sessionResult = await connection.execute(sessionSql, { userId: decoded.userId, refreshToken: refreshToken });
    const sessionResultRow: sessionRow = sessionResult.rows?.[0] as sessionRow || [];
    ...
    const userResult = await connection.execute(userSql, { userId: decoded.userId });

    // 取第一行数据
    const userResultRow: refreshUserRow = userResult.rows?.[0] as refreshUserRow || [];
